...
test('Can get a used price guide', t => {
    return BL.getPriceGuide(ItemType.Set, '10224-1', {new_or_used: Condition.Used})
      .then(guide => {
          t.is(guide.item.no, '10224-1', 'Got the right item back');
          t.true(guide.max_price > 0, 'Returns a max price');
          t.true(guide.min_price > 0, 'Returns a min price');
...
test('Can get information on a book', t => {
    return BL.getCatalogItem(ItemType.Book, '810002')
      .then(book => {
...
test('Can get information on a set', t => {
    return BL.getCatalogItem(ItemType.Set, '10224-1')
      .then(set => {
...
test('Can get information on minifigure', t => {
    return BL.getCatalogItem(ItemType.Minifigure, 'pi055')
      .then(fig => {
...
test('Can get known colors for a part', t => {
  return BL.getKnownColors(ItemType.Part, '3001')
    .then(colors => {
...
test('Can get an image for a known color id of a catalog item', t => {
  return BL.getItemImage(ItemType.Part, '3001', 1)
    .then(image => {
...
test('Can get an inventory of a minifigure', t => {
  return BL.getItemSubset(ItemType.Minifigure, 'pi055')
    .then(subset => {
      t.true(subset.length > 0, 'Returns a list subset items');
       t.true(subset[0].entries.length > 0, 'Returns a list of entries with each subset item');
    });
});

test('Can get an inventory of a set', t => {
  return BL.getItemSubset(ItemType.Set, '6024-1')
    .then(subset => {
       t.true(subset.length > 0, 'Returns a list subset items');
       t.true(subset[0].entries.length > 0, 'Returns a list of entries with each subset item');
    });
...
test('Can get an inventory of a part', t => {
  return BL.getItemSubset(ItemType.Part, '4592c04')
    .then(subset => {
       t.true(subset.length > 0, 'Returns a list subset items');
       t.true(subset[0].entries.length > 0, 'Returns a list of entries with each subset item');
    });
...
test('Can display what other parts a part is in', t => {
  return BL.getItemSuperset(ItemType.Part, '4593')
    .then(superset => {
        t.true(superset.length > 0, 'Returns a list superset items');
        t.true(superset[0].entries.length > 0, 'Returns a list of entries with each superset item');
    });
});
